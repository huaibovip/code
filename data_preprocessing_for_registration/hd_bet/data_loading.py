...
def preprocess_image(itk_image, is_seg=False, spacing_target=(1, 0.5, 0.5)):
    ...
    assert (
        len(image.shape) == 3
    ), "The image has unsupported number of dimensions. Only 3D images are allowed"
    ...
    if not is_seg:
        ...
        new_shape = (
            int(np.round(spacing[0] / spacing_target[0] * float(image.shape[0]))),
...
def load_and_preprocess(mri_file):
    ...
    for k in images.keys():
        images[k] = preprocess_image(
            images[k], is_seg=False, spacing_target=(1.5, 1.5, 1.5)
        )
...
def save_segmentation_nifti(segmentation, dct, out_fname, order=1, dtype=np.uint8):
    ...
    if np.any(
        [
            i != j
            for i, j in zip(np.array(seg_old_size), np.array(dct["size"])[[2, 1, 0]])
        ]
    ):
