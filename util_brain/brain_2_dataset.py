...
from brain_1_yaml import get_config_from_from_yaml
...
def make_png_dataset(cfg_path, src_root, dst_root):
    ...
    for phase, subjects in dataset.items():
        phase_root = os.path.join(dst_root, 'brain_' + modals_name.lower(), phase)
        ...
        for idx, sub in subjects.items():
            ...
            modal_paths = {m: os.path.join(src_root, name, m, name + '_' + m + '_{}.npy') for m in modals}
...
def make_hdf5_dataset(cfg_path, src_root, dst_root):
    ...
    for phase, subjects in dataset.items():
        ...
        os.makedirs(phase_root, exist_ok=True)
        f = h5py.File(os.path.join(phase_root, f'brain_{modals_name.lower()}_{phase}.hdf5'), 'w')
...
if __name__ == '__main__':
    ...
    func = 'make_' + DATASET_TYPE + f'_dataset(\'brain_{TYPE}_config.yaml\', src_root, dst_root)'
    eval(func)
